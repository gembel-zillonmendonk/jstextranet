 ...
 * @author farid
 */
class Ep_pgd_penawaran_adm extends MY_Model
{
    ...
    public $validation = array(
        'VENDOR_CEK' => array('required' => true),
    );
    public $form_view = 'pengadaan/form_adm';
    ...
    function __construct()
    {
        ...
        if (count($rows) > 0)
        {
            $this->elements_conf['VENDOR_CEK']['value'] = array();
            foreach ($rows as $v)
            {
                $this->elements_conf['VENDOR_CEK']['options'][] = $v;
                $this->elements_conf['VENDOR_CEK']['value'][] = $v['VENDOR_CEK'];
            }
        }
        else
        {
            ...
            $this->elements_conf['VENDOR_CEK']['options'] = array();
            foreach ($rows as $v)
            {
                $this->elements_conf['VENDOR_CEK']['options'][] = $v;
            }
