...
class monitoring extends MY_Model {
    ...
    public $sql_select = "( select a.*, x.kode_proses, x.nama_aktifitas, x.url, '' as ACT
                from (
                    select a.* ,c.NAMA_AKTIFITAS, b.url, d.KODE_PO, KODE_KONTRAK, KODE_KANTOR, KODE_VENDOR, KODE_TENDER, KODE_PERKEMBANGAN, KODE_INVOICE
                    ...
                    where kode_wkf = 64 
                ) x inner join EP_KTR_PO a 
                on x.KODE_VENDOR = a.KODE_VENDOR
                and x.KODE_KANTOR = a.KODE_KANTOR
                and x.KODE_KONTRAK = a.KODE_KONTRAK
                and x.KODE_PO = a.KODE_PO
            )";
    ...
    function __construct() {
		...
		for(var i=0;i < ids.length;i++){
                    ...
                    var param = "referer_url=/contract/po/monitoring&KODE_KONTRAK=" + data[\'KODE_KONTRAK\'] + 
