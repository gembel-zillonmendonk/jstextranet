...
//                array('model' => 'vendor/ep_vendor_sertifikat', 'label' => 'KETERANGAN SERTIFIKAT', 'rules' => 'hasOne', 'where' => $this->where),
    ...
    public function login() {
        if (count($_POST) > 0) {
            ...
            $query = $this->db->query("select NAMA_VENDOR, KODE_VENDOR, KODE_LOGIN, PASSWRD from EP_VENDOR where KODE_LOGIN = '$username' 
                    and status in (1,2,5,9,-4)");
            $row = $query->row();
    ...
    public function registration() {
        if (count($_POST) > 0) {
            try {
                ...
                $_POST['EP_VENDOR']['STATUS'] = '1';
                $_POST['EP_VENDOR']['KODE_STATUS_REG'] = '3';
                $_POST['EP_VENDOR']['STATUS_REG'] = 'Fill Registration Form';
    ...
    public function fix_registration() {
        ...
        if ($proses) {
            $this->layout->setLayout('layout_nomenu');
            $this->layout->view('fix_registration', array('proses' => $proses));
            //modules::run('wkf/run?referer_url=/account/waiting_approval&kode_proses='.$proses['KODE_PROSES'].'&KODE_VENDOR='.$proses['KODE_VENDOR']);
            //redirect('wkf/run?referer_url=/account/waiting_approval&kode_proses='.$proses['KODE_PROSES']);
        }
    }
...
//                $this->db->query("update ep_nomorurut set NOMORURUT = NOMORURUT + 1 where kode_nomorurut = 'VENDOR'");
        ...
        $this->layout->view('account/change_password');
    }
